...
A due `scheduled` cron occurrence is flipped directly to `running` with no separate
promotion step, whose downtime would reintroduce scheduler-down blindness. All time
comparisons evaluate DB-side against Postgres `now()`.
"""
...
async def claim_runs(
    ...
    for run in runs:
        ...
        # (ADR-0021) drives the same increment for genuine failures.
        prior = (
