...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
    ...
    try {
        ...
        if (!implement.commits.length) {
            console.log(
                "Implementation agent made no commits. Skipping review.",
            );
            continue;
        }
        ...
        // Phase 3: Open PR
        //
        // A final agent pushes the branch and opens a pull request against the
        // source branch with a concise description (filling a PR template if the
        // repo has one). The issue is linked via "Closes #N" rather than closed.
        // -----------------------------------------------------------------------
        await sandbox.run({
            name: "pr-author",
            maxIterations: 5,
